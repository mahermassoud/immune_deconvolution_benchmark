...
sim.methods <- config$deconvolution_methods[
  config$deconvolution_methods != "eigengene"]
  
timer_indications = rep("OV", ncol(schelker_ovarian$expr_mat))
all_results_bulk = foreach(method=sim.methods,
                           .final=function(x) {setNames(x, sim.methods)}) %do% {
  deconvolute(schelker_ovarian$expr_mat, method, indications=timer_indications) %>%
    mutate(method=method) %>%
    mutate(source="bulk")
}

all_results_simulated = foreach(method=sim.methods,
                                .final = function(x) {setNames(x, sim.methods)}) %do% {
...
all_results = bind_rows(bind_rows(all_results_bulk), bind_rows(all_results_simulated)) %>%
